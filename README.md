...
工作台为浅灰侧栏 + 纸面色工作区，侧栏只有「近期处理 / 全部订阅」两个视图；数据备份与退出登录收在右上角「⋯」菜单里，手机底栏同样只保留两个视图。近期页保留三项统计（未来 30 天预计续费、逾期未确认、月度等效预算），清单按逾期 / 7 天内 / 30 天内分组并显示组内每期金额小计；逾期与 7 天内的记录行内直接提供「记录续费」。全部页直接显示清单，工具栏固定、列表独立滚动，支持名称/备注搜索、状态筛选、组内排序与分组折叠（已取消、已结束默认折叠）。整行可点击打开详情；宽屏详情与列表并排，窄屏为原生模态。金额千分位显示，日期同时给出「逾期/还剩 N 天」和「9月8日 周二」读法，记录本身仍为 ISO 日期。快捷键：`/` 聚焦搜索，`N` 新增订阅，`Esc` 关闭详情。删除与恢复备份使用页面内确认弹窗并写明影响的记录数。URL hash 保留视图和选中记录，支持刷新及浏览器返回。所有图标与资源均在本地，深色模式随系统。
