...
工作台采用固定导航与全宽清单，提供「近期处理 / 全部订阅 / 数据备份」三个视图。近期页保留统计，每个订阅仅显示一次；统计仍按周期展开预测。全部页直接显示清单，工具栏固定、列表独立滚动。支持名称/备注搜索、状态筛选、组内排序与分组折叠。宽屏详情与列表并排，窄屏采用原生模态弹窗，手机使用底部导航。URL hash 保留视图和选中记录，支持刷新及浏览器返回。已取消与已结束分组默认折叠，搜索时展开匹配组。所有图标与资源均在本地。

页面导出 JSON 包含订阅及续费历史，不包含登录密码/密钥。`version=1` 格式不变，新版支持恢复旧有月付/年付/天数备份以及含季付的备份；旧版应用不支持含 `quarterly` 的备份。备份仍含私人备注，需安全存放。
